...
/**
 * Fluid VaultResolver, as deployed (verified 2026-09-16 by decoding a raw
 * getVaultEntireData call: 97 words). The current source adds a trailing
 * `uint8 accessType` that the live resolver does not return, so it is omitted.
 */
export const FLUID_VAULT_RESOLVER_ABI = parseAbi([
  ...
  "struct VaultState { uint256 totalPositions; int256 topTick; uint256 currentBranch; uint256 totalBranch; uint256 totalBorrow; uint256 totalSupply; CurrentBranchState currentBranchState; }",
  ...
  "struct VaultEntireData { address vault; bool isSmartCol; bool isSmartDebt; ConstantViews constantVariables; Configs configs; ExchangePricesAndRates exchangePricesAndRates; TotalSupplyAndBorrow totalSupplyAndBorrow; LimitsAndAvailability limitsAndAvailability; VaultState vaultState; UserSupplyData liquidityUserSupplyData; UserBorrowData liquidityUserBorrowData; }",
  "struct UserPosition { uint256 nftId; address owner; bool isLiquidated; bool isSupplyPosition; int256 tick; uint256 tickId; uint256 beforeSupply; uint256 beforeBorrow; uint256 beforeDustBorrow; uint256 supply; uint256 borrow; uint256 dustBorrow; }",
  "function getAllVaultsAddresses() view returns (address[])",
  "function getVaultsEntireData(address[] vaults_) view returns (VaultEntireData[])",
  "function positionsByUser(address user_) view returns (UserPosition[], VaultEntireData[])",
]);
